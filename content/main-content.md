---
title: 'Craig Bassett - Resume'
description: 'My Resume'
---

::Section{Title=Summary ContentClass="text-neutral-600 dark:text-neutral-400"}

<!-- TODO:
- Make sure its printable.
- Github Actions to build, test and deploy
- Fix the tab icon, maybe my pic or something random? -->

Senior front end full stack software engineer specializing in user experience, user interface, and general well being of the greater web.

Experience spans the majority product lifecycle - from creative to design, through to development, testing and support - coupled with team management.

Further experience with several advanced content management systems, along with social networking integration and mobile development.

High performance, scalable, well architected and executed solutions are always strived for.
Extra care and attention is applied toward process, tools and continuous integration, and delivery.
::

::Section{Title="Work Experience"}
  ::Experience{Company="Quest Software" Website="https://quest.com" Dates="2016 - Present" JobTitle="Principal Software Engineer"}
    - Created a new Licensing UI
      - Designed and implemented UX and UI
      - Originally written in Vue2, and re-written in Vue3
      - Defined API endpoints and requirements
    - Helped manage, define, build, and deliver a new component library
    - Defined and built an icon library
      - Included multiple avenues of consumption to allow any team to use
      - Consumption paths included npm, cloning the source or downloading svg's for use in marketing material
    - Developed a new Angular 2 App
      - Developed a NodeJS Express API
      - Included gulp pipelines, tests and associated work in order to nurture continuous integration and delivery

    Notable achievements;

    - New UI Framework learnings, Vue, Vuetify, Quaser, Vue-Query
    - Successfully completed transition to Quest Software from Dell Software.
    - Further learnings with Angular2 and TypeScript.
    - Alignment with legacy .NET codebase.
  ::

  ::Experience{Company="Dell" Website="https://dell.com" Dates="2014 - 2016" JobTitle="Principal Software Engineer"}
    - Manage, define and police all code in the RESTful component of the Dell Cloud Manager product. This component provides the data and interface to and from the User Interface and backend components.
    - Contributing to Dell's best practice for RESTful API's and implementation requirements given our technical platform.
    - Serve as the Technical Manager for front end engineering and operations as a point of contact with the back end, product and quality teams. I liaise with other team leads to manage daily features, bugs and general workflows. Product roadmaps and planning are also discussed daily.
    - Possess a rare and intimate knowledge of the Cloud and Dell Cloud Manager across the full technology stack, and serve as a point of contact for all teams when required - especially where concerning the RESTful API or Angular UI applications. 
    - Oversee and manage Creative Design requirements in the product - which involves a direct and intimate relationship with the UX and Product Management teams. This particular function ensures that the vision prescribed by UX is met in the final delivery of changes by our team.

    Notable achievements;

    - Moved countries!
  ::

  ::Experience{Company="Dell" Website="https://dell.com" Dates="2013 - 2014" JobTitle="Principal Software Engineer"}
    - Successfully completed transition of acquisition into Dell.
    - Continued rewriting and implementing new front end components of the now Dell Cloud Manager product.
    - Further redesign and changes were implemented in order to meet the Dell greater brand requirements.
  ::

  ::Experience{Company="Enstratius" Website="https://en.wikipedia.org/wiki/Enstratius" Dates="2011 - 2013" JobTitle="Frontend Software Architect"}
    - Redesigned the entire app interface given new IA requirements.
    - Introduced, implemented and helped to migrate the team from SVN to GIT version control.
    - Rewrote front end components of application layer using new technologies.
    - Implemented a new RESTful API for communication with legacy backend systems in JAVA using the GRAILS framework.
    - Implemented best practices for code separation and basic OO principles.
    - Implemented front end interactive features such as navigation, core UI components and other key elements to ensure UX and UI is consistent and usable.

    Notable achievements;

    - A new interface design.
    - A new backend API.
  ::

  ::Experience{Company="APN Online" Website="https://www.nzherald.co.nz" Dates="2009 - 2011" JobTitle="Front End Engineer"}
    - User Interface and User Experience advice, direction and implementation.
    - Creative thought, recommendations and execution of front end design against existing codebases.
    - Re-engineering existing code to meet new standards.
    - Integration of GIT as version control, migrated from SVN - along with documentation and training.

    Notable achievements;

    - The design and implementation of a new video interface
    - Design feedback and implementation of the news layout for the homepage and associated categories.
    - Design and implementation of a new mobile interface.
  ::

  ::Experience{Company="Spitfire Creative" Website="https://www.spitfire.co.nz" Dates="2006 - 2009" JobTitle="Technical Director"}
    - Managed, maintained and implemented brochureware and competition sites
  ::

  ::Experience{Company="Ogilvy Interactive" Website="https://www.ogilvy.co.nz" Dates="2005 - 2006" JobTitle="Senior Developer"}
    - Implemented infrastructure and technical requirements for ongoing projects. This included initial creative thought through to database design and slicing of designs along with deployment and management of production environments.
  ::

  ::Experience{Company="Walkers Interactive" Dates="2000 - 2005" JobTitle="Senior Web Developer"}
    - Implemented infrastructure and technical requirements for ongoing projects. This included initial creative thought through to database design and slicing of designs along with deployment and management of production environments.
  ::
::
